"""Post-C5.36 guarded knowledge-to-explanation certification for Star Comprehensive.

Historical MO-023H certification remains unchanged. This successor certifies the
same narrow Star Comprehensive conditional co-payment scope only when the
current request-authority, identity, evidence, decision, explanation and
rendering guards are demonstrably exercised.
"""
...
class GuardedFullCycleCertificationError(ValueError):
    """Raised when guarded full-cycle certification cannot complete safely."""
...
class GuardedFullKnowledgeToExplanationResult:
...
def validate_guarded_response(response: GuardedStarComprehensivePilotResult) -> None:
    if not isinstance(response, GuardedStarComprehensivePilotResult):
        raise GuardedFullCycleCertificationError("response is not a guarded Star pilot result")
    if response.guard_status != "GUARDED":
        raise GuardedFullCycleCertificationError("guarded response status is missing")
    if response.instance_sufficiency.outcome != "PASS" or not response.instance_sufficiency.planning_authorized:
        raise GuardedFullCycleCertificationError("guarded response did not pass Instance Sufficiency")
    if (
        ...
        raise GuardedFullCycleCertificationError("guarded response did not exercise Evidence Instance Enforcement")
    if (
        ...
        raise GuardedFullCycleCertificationError("guarded response did not exercise Authority-Enforced Decision")
    if response.render_conformance.outcome != "PASS" or response.render_conformance.rendered_text is None:
        raise GuardedFullCycleCertificationError("guarded response did not pass Rendering Exit Safety")
    if response.identity_resolution.status != "RESOLVED" or not response.identity_record_hash:
        raise GuardedFullCycleCertificationError("guarded response did not preserve governed product identity")


def run_guarded_full_knowledge_to_explanation_certification(
    *,
    ...
    """Re-earn certification only through the current guarded response path."""
    root = Path(repository_root).resolve()
    if not root.is_dir():
        raise GuardedFullCycleCertificationError("repository_root must be an existing directory")
    if not isinstance(response_request_id, str) or not response_request_id.strip():
        raise GuardedFullCycleCertificationError("response_request_id must be a non-empty string")
    if not isinstance(question, str) or not question.strip():
        raise GuardedFullCycleCertificationError("question must be a non-empty string")
    ...
    try:
        ...
        raise GuardedFullCycleCertificationError(f"knowledge certification failed: {exc}") from exc

    if build.status != "CERTIFIED":
        raise GuardedFullCycleCertificationError("knowledge build did not produce a certified snapshot")
    if build.product_reference != PRODUCT_REFERENCE or build.topic != TOPIC:
        raise GuardedFullCycleCertificationError("certified knowledge scope does not match guarded response pilot")
    if not build.knowledge_snapshot_id:
        raise GuardedFullCycleCertificationError("certified knowledge snapshot identity is missing")
    ...
    try:
        ...
        raise GuardedFullCycleCertificationError(f"guarded response certification failed: {exc}") from exc

    validate_guarded_response(response)
    if response.product_reference != build.product_reference or response.topic != build.topic:
        raise GuardedFullCycleCertificationError("guarded response scope diverged from certified knowledge scope")
    if response.knowledge_snapshot_id != build.knowledge_snapshot_id:
        raise GuardedFullCycleCertificationError("guarded response did not consume the certified snapshot")
    if response.released_response_id != response.response.response_id:
        raise GuardedFullCycleCertificationError("guarded released response identity is inconsistent")
    ...
    return GuardedFullKnowledgeToExplanationResult(
