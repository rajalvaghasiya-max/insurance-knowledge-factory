...
This module is intentionally product-specific orchestration over generic certification and
publication-decision machinery. It does not add reasoning rules, broaden product-level
co-payment authority, or authorize policy-specific option selection.
"""
...
def build_bajaj_lab_copay_publication_decision(
    *, repository_root: str | Path
    ...
    retained_limitations = tuple(
        item for item in certification.limitations if "bound_not_published" not in item.casefold()
    )
    evidence_refs = tuple(package.evidence_id for package in case.evidence_output.evidence_packages)
    return evaluate_publication_decision(
        build_publication_decision_input(
            ...
            decision_reasons=(
                ...
                "The publication remains scoped to the lab/radiology reimbursement-not-pre-approved condition and does not establish a product-level co-payment.",
            ),
