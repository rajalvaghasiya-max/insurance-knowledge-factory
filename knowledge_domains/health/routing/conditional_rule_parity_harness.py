...
This module is intentionally a transitional certification aid, not production routing logic.
It lives under ``knowledge_domains`` because it adapts legacy Health triage into the current
generic rule assembler for parity verification. It must not be imported by authoritative
``factory_core`` or ``insurance_intelligence`` production code.
"""
...
class CopayAssemblyParityHarness:
    ...
    The harness treats a mismatch as a finding, not as a reason to coerce output.
    It exists only to verify transitional parity before legacy paths are retired.
    """
