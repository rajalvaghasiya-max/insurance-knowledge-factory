...
from knowledge_domains.health.extractors.room_rent_extractor import RoomRentExtractor
...
def main():
    parser = argparse.ArgumentParser(
        description="Run evidence-backed room rent extraction."
    )
    ...
    parser.add_argument(
        "--search-root",
        default="parsed/html_sections/aditya_birla_health",
        help="Folder to scan. Prefer product/insurer scoped folder.",
    )

    parser.add_argument(
        "--no-entity-guard",
        action="store_true",
        help="Disable entity/path guard. Not recommended for broad roots.",
    )
    ...
    output_path = output_dir / f"{safe_entity_id(entity_id)}_room_rent_limit.json"
