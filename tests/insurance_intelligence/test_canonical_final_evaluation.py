...
from insurance_intelligence.contracts.evaluation import build_assertion, build_scenario
...
from insurance_intelligence.evaluation.canonical_final import execute_canonical_final_evaluation
...
def _request():
    return build_orchestration_request(
        execution_id="real-final-evaluation-star-ped-case-b",
...
def _scenario():
    return build_scenario(
        scenario_id="star-ped-canonical-final-evaluation",
        scenario_version="1.0",
        name="Canonical factual answer final evaluation",
        description="Evaluate the released deterministic factual answer captured from the canonical guarded runtime.",
        scenario_kind="GENERAL_EXPLANATION",
        request_text="What is the PED waiting period in Star Comprehensive?",
        domain="health",
        topic="waiting_period",
        audience="CUSTOMER",
        expected_response_statuses=("ANSWER", "ANSWER_WITH_LIMITATIONS"),
        stage_expectations=(),
        assertions=(
            build_assertion(
                assertion_id="canonical-response-status",
                category="RESPONSE_STATUS",
                description="Released response must remain an answer status.",
                expected_values=("ANSWER", "ANSWER_WITH_LIMITATIONS"),
            ),
            build_assertion(
                assertion_id="canonical-audience-format",
                category="AUDIENCE_FORMAT",
                description="Released response must preserve customer standard format.",
                expected_values=("CUSTOMER", "STANDARD"),
            ),
        ),
        tags=("canonical", "final-evaluation", "factual-lane"),
    )


def test_star_ped_real_path_reaches_final_evaluation_without_private_pipeline_replay():
    request = _request()
    ...
    def forbidden_renderer(**kwargs):
        ...
        raise AssertionError("renderer must not be invoked when rendering is disabled")

    run = execute_canonical_final_evaluation(
        request=request,
        dependencies=dependencies,
        style_registry=_styles(),
        response_registry=_responses(),
        rendering_capability=forbidden_renderer,
        evaluation_scenario=_scenario(),
    )

    assert tuple(item.stage for item in run.stage_results) == request.requested_stage_order
    assert run.stage_results[-2].stage == "LLM_RENDERING"
    assert run.stage_results[-2].status == "NOT_REQUIRED"
    assert run.stage_results[-1].stage == "FINAL_EVALUATION"
    assert run.stage_results[-1].status == "SUCCEEDED"
    assert renderer_calls == []
    assert run.blocked is False
    assert run.evaluation_result is not None
    assert run.evaluation_result.outcome == "PASS"
    assert run.evaluation_result.failed_assertion_ids == ()

    assert run.released_response_id is not None
    response = dependencies.store.get(run.released_response_id)
    ...
    evaluation_output_id = run.stage_results[-1].outputs[0].output_id
    assert dependencies.store.get(evaluation_output_id) is run.evaluation_result
