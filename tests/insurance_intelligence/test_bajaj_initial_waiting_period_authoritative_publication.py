...
PUBLICATION_CASE_PATH = Path(
    ...
    "generic_rule_certification/initial_waiting_period_publication_case.json"
)
...
def _build_publication_chain():
    ...
    certification = run_rule_certification(
        ...
        domain=case.domain,
        trace_references=("certification:bajaj_my_health_care_initial_waiting_period",),
    )
...
def test_bajaj_initial_wait_publication_case_is_data_only_and_current_source_bounded():
    ...
    assert any(
        "does not publish PED" in guardrail
        for guardrail in spec["guardrails"]
    )
    assert any(
        "product-wide governed readiness" in guardrail
        for guardrail in spec["guardrails"]
    )


def test_bajaj_initial_wait_reaches_publish_decision_without_dropping_limitations():
    ...
    assert any(
        "unrelated Bajaj waiting-period residue remains unresolved" in item
        for item in decision.limitations
    )


def test_bajaj_initial_wait_creates_bounded_authoritative_publication():
    _, case, _, decision, projection, publication = _build_publication_chain()
    ...
    assert publication.certification_id == "bajaj_my_health_care_initial_waiting_period"
    assert publication.topic_id == "waiting_period"
