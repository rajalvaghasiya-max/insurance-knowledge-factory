...
def _run_prefix(question: str, *, execution_id: str):
    request = _request(question, execution_id=execution_id)
    dependencies = _dependencies(request)
    adapters = build_real_response_prefix_adapters(dependencies=dependencies)
    assert tuple(adapter.stage for adapter in adapters) == request.requested_stage_order[:9]
...
def test_real_prefix_composes_existing_guarded_stages_without_fake_capabilities():
    request, dependencies, results = _run_prefix(
        "Explain Star Comprehensive",
        execution_id="real-prefix-product-explanation",
    )

    assert len(results) == 9
    assert results[-1].stage == "EVIDENCE_RESOLUTION_ENFORCED"
    evidence = dependencies.store.get(results[-1].outputs[0].output_id)
    assert evidence.request_id == request.execution_id
    assert evidence.resolution_status in {"RESOLVED", "RESOLVED_WITH_LIMITATIONS"}
    assert evidence.evidence_packages


def test_star_ped_case_b_reaches_publication_backed_evidence_through_same_real_prefix():
    request, dependencies, results = _run_prefix(
        "What is the PED waiting period in Star Comprehensive?",
        execution_id="real-prefix-star-ped-case-b",
    )
    ...
    assert evidence.resolution_status in {"RESOLVED", "RESOLVED_WITH_LIMITATIONS"}
    assert evidence.evidence_packages
    assert any("waiting" in item.field_or_topic.lower() for item in evidence.evidence_packages)
