...
def _responses():
    return ResponseFormatRegistry((
        build_format_definition(
            ...
            section_order=("EDUCATION", "DIRECT_ANSWER", "EXAMPLE", "EXPLANATION", "CONDITION", "LIMITATION", "EVIDENCE"),
            allowed_section_types=("EDUCATION", "DIRECT_ANSWER", "EXAMPLE", "EXPLANATION", "CONDITION", "LIMITATION", "EVIDENCE"),
...
def test_d0_case_a_target_checkpoint_has_education_primary_fact_and_safe_example() -> None:
    ...
    included = tuple(
        section for section in response.sections if section.status == "INCLUDED"
    )
    section_types = tuple(section.section_type for section in included)
    assert "EDUCATION" in section_types
    assert "DIRECT_ANSWER" in section_types
    assert "EXAMPLE" in section_types
    assert section_types.index("EDUCATION") < section_types.index("DIRECT_ANSWER")

    education = next(
        section.text for section in included if section.section_type == "EDUCATION"
    )
    ...
    example = next(
        section.text for section in included if section.section_type == "EXAMPLE"
    )
    assert "ear condition" in example.lower()

    assert projection.human_view.answer == "The waiting period duration is 36 MONTHS."
    assert any(
        "health problem or medical condition" in text.lower()
        for text in projection.human_view.meaning
    )


def test_d0_case_a_target_checkpoint_requires_practical_product_scenarios() -> None:
    _, _, response = _run_real_star_ped_response(with_education=True)
    projection = project_human_answer(response)
    customer_text = " ".join(
        (
    ...
    # Founder-approved target pressure:
    # same PED during wait, unrelated new illness, and same PED after wait.
    assert "10 months" in customer_text
    assert "dengue" in customer_text
    assert "40 months" in customer_text
