...
from insurance_intelligence.orchestration.real_response_decision import build_real_response_decision_adapters
...
def test_star_ped_factual_lane_reaches_authority_enforced_decision_without_new_semantics():
    request = _request(
        "What is the PED waiting period in Star Comprehensive?",
        execution_id="real-decision-star-ped-case-b",
    )
    dependencies = _dependencies(request)
    adapters = build_real_response_decision_adapters(dependencies=dependencies)
    assert tuple(adapter.stage for adapter in adapters) == request.requested_stage_order[:11]
    ...
    assert results[-1].stage == "DECISION_GATE_AUTHORITY_ENFORCED"
    enforcement = dependencies.store.get(results[-1].outputs[0].output_id)
    assert enforcement.enforcement_outcome == "DELEGATED_TO_DECISION_GATE"
    assert enforcement.decision_gate_called is True
    assert enforcement.recommendation_authorized is False
    assert enforcement.decision_output is not None
    assert enforcement.decision_output.decision in {"APPROVED", "APPROVED_WITH_LIMITATIONS"}
    assert enforcement.decision_output.response_packet.approved_finding_ids
    assert enforcement.decision_output.response_packet.approved_evidence_ids
