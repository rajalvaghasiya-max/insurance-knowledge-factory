...
def _waiting_period_record() -> dict:
    return GovernedGenericConceptRecordContract.create_record(
        ...
        meaning_profile={
            ...
            "trigger": (
                "The relevant waiting-period rule is evaluated when a policy event is "
                "tested against its governed start basis, duration, scope, and exceptions."
            ),
            "inputs": [
                "applicable_start_basis",
                "waiting_period_duration",
                "event_date",
                "applicability_scope",
                "continuity_or_credit",
                "exceptions",
            ],
            "outputs": [
                "waiting_period_applicability_state",
            ],
            "calculation_basis": (
                "Any timeline calculation must use the governed start basis, duration, "
                "and authorized boundary convention. No exact boundary convention is "
                "invented when the governing evidence does not establish one."
            ),
            "dependencies": [
                "policy_terms",
                "applicable_start_basis",
            ...
            "exceptions": [
                "No generic exception is asserted. Any exception must come from governed "
                "product and customer-document evidence."
            ],
        },
        simple_example={
            "scenario": (
        ...
        source_evidence=[
            {
                ...
                "evidence_text": (
                    ...
                    "genericity testing only."
                ),
                "evidence_scope": "generic_insurance_concept",
            }
        ],
        review_decision={
            "review_decision_id": "review_waiting_period_genericity_repair_v1",
            "decision": "approve_for_governed_generic_concept_creation",
            "reviewer_identity": "test-reviewer",
            "reviewed_at": "2026-09-20T00:00:00Z",
            "rationale": (
                "Approved as a governed third-concept fixture to prove the meaning "
                "adapter consumes concept semantics from data rather than runtime branches."
            ),
        },
        knowledge_version="genericity-repair-v1",
        created_by="test",
...
def test_third_governed_concept_builds_without_adapter_tuning() -> None:
    ...
    assert asset["category"] == "coverage_activation_timeline"
    ...
    assert asset["trigger"] == record["meaning_profile"]["trigger"]
    assert asset["inputs"] == record["meaning_profile"]["inputs"]
    assert asset["outputs"] == record["meaning_profile"]["outputs"]
    assert asset["policy_examples"][0]["source_example"] == record["simple_example"]

    rendered_example = asset["policy_examples"][0]["result"].lower()
    assert "claim approval" in rendered_example
    assert "claim payment" in rendered_example
    assert "exact calendar boundary" in rendered_example
