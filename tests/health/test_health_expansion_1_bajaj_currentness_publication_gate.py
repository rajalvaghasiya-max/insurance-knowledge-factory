...
DOCUMENT_VERSION_ID = "docver_bajaj_my_health_care_policy_wording_v1_05dc291324340d52"
...
SOURCE = {
    ...
    "source_page_url": (
        ...
        "my-health-care-plan.html"
    ),
    "relative_archive_path": (
        "archive/raw_pdf/bajaj_allianz_general/policy_wording/"
        "My-Health-Care-Plan1-PW__05dc29132434.pdf"
    ),
...
def _materialization():
    review = {
        ...
        "review_groups": [{
            "group_id": "crgrp_bajaj_health_expansion_1",
            "candidate_type": "currency_amount",
            "normalized_value": {"kind": "currency", "value": 50000, "unit": "INR"},
            "inferred_scope": {
                "benefit_scope_key": "health_expansion_1_pressure_only",
                "band_scope_key": None,
            },
            "review_flags": ["role_selection_required"],
            "supporting_candidates": [{"candidate_id": "excand_bajaj_health_expansion_1"}],
        }],
    }
    ...
    pending["decision_records"][0] = ReviewerDecisionRecordContract.build_resolved_record(
        ...
        review_rationale="HEALTH-EXPANSION-1 publication-gate pressure fixture.",
        selected_role="sub_limit_or_limit",
        selected_benefit_scope="health_expansion_1_pressure_only",
    ...
    selection = GovernedFactSelectionContract.build_selection_document(
        ...
        selected_at="2026-08-15T10:10:00Z",
    )
    return CanonicalFactMaterializationContract.build_materialization_document(
        selection,
        materialized_by="materializer@policyscna.test",
        materialized_at="2026-08-15T10:15:00Z",
    )


def _overlay(*, temporal_status: str, current_entitlement: str):
    return {
        "schema_version": "1.0",
...
def test_bajaj_05dc_compatibility_unverified_blocks_publication_review():
    result = FactPublicationEligibilityContract.build_eligibility_document(
        _materialization(),
        _overlay(temporal_status="compatibility_unverified", current_entitlement="blocked"),
        validated_by="validator@policyscna.test",
        validated_at="2026-08-15T10:20:00Z",
    )

    assert result["eligible_for_publication_review_count"] == 0
    assert result["blocked_count"] == 1
    assert result["eligibility_records"][0]["eligibility_status"] == "blocked"
...
def test_bajaj_05dc_current_reviewed_becomes_eligible_for_publication_review():
    result = FactPublicationEligibilityContract.build_eligibility_document(
        _materialization(),
        _overlay(temporal_status="current_observed_reviewed", current_entitlement="eligible"),
        validated_by="validator@policyscna.test",
        validated_at="2026-08-15T10:20:00Z",
    )
...
def test_bajaj_currentness_transition_never_auto_publishes_fact():
    blocked = FactPublicationEligibilityContract.build_eligibility_document(
        _materialization(),
        _overlay(temporal_status="compatibility_unverified", current_entitlement="blocked"),
        validated_by="validator@policyscna.test",
        validated_at="2026-08-15T10:20:00Z",
    )
    eligible = FactPublicationEligibilityContract.build_eligibility_document(
        _materialization(),
        _overlay(temporal_status="current_observed_reviewed", current_entitlement="eligible"),
        validated_by="validator@policyscna.test",
        validated_at="2026-08-15T10:20:00Z",
    ...
    assert blocked["publication_state"] == "not_published"
    assert eligible["publication_state"] == "not_published"
    assert all(
        record["publication_state"] == "not_published"
        for record in eligible["eligibility_records"]
    )
